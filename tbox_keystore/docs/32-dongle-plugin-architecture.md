...
1. **运行期加载**：CA 启动时扫描插件目录、`dlopen` 加载，增减后端无需重编译
...
5. **每设备独立身份 + 远端策略 + 审计日志**
6. **保留现有选择方式**：`--dongle <name>` 显式指定；无参时自动探测

### 2.2 非目标（本期明确不做）

| 不做 | 说明 |
|------|------|
| **运行中热插拔** | 仅**启动时扫描一次**；运行中放入/移除需重启 |
...
/* 可选：加载器把"插件搜索目录"告知插件（便于本地软狗解析配套 .key） */
...
```c
struct dongle_ops {
	const char *name;       /* 唯一标识，--dongle <name> 用它匹配   */
	uint32_t    caps;       /* DONGLE_CAP_*                        */
...
排序：**priority 降序 → 同 priority 按插件文件名升序**（结果确定，不依赖 `readdir` 顺序）。
...
**命名约定**：`<name>.so` 与配套密钥 `<name>.key` 同名。
...
| 6 | `--dongle dummy` / `--dongle remote` | 精确匹配 |
| 7 | ABI 版本不匹配的 `.so` | **拒绝加载并告警**，其他插件不受影响 |
...
| # | 场景 | 期望 |
|:--:|------|------|
| 11 | 远端可达且设备已授权 | `probe()==1`；`sign()` 成功 |
| 12 | 远端不可达（拔网线/关服务） | `probe()==0`（**2s 内返回**，不卡死） |
| 13 | 设备未登记（公钥不在 `authorized_keys`） | SSH 拒绝 → 视为未插入 |
| 14 | 远端策略拒绝（不在 `devices.json`） | 返回 denied，**审计日志有记录** |
| 15 | 篡改 `known_hosts` 模拟 MITM | **连接被拒绝**（主机密钥校验失败） |
| 16 | 网络中途断开 | 报错清晰，**不回退到本地软狗** |
...
# mv "$DGN/dummy.so"  /tmp/          # 拿走驱动 → 插件根本不加载
...
| 2 | **插件目录最终路径** | 暂定 `/usr/lib/tbox/dongle/` |
