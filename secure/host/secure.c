...
int32_t CryptoRsaEnc(uint8_t *id, uint32_t idLen, const BUFFER inbuf, BUFFER *outbuf)
{
    ...
    if (!op.params[2].tmpref.buffer) {
        teec_err(1, eo, "Cannot allocate out buffer of size");
    }
...
int32_t CryptoShaHash(void *inbuf, size_t inbuf_len, void **outbuf, size_t *outbuf_len)
{
    TEEC_Result res;
    TEEC_Operation op;
    memset(&op, 0, sizeof(op));
...
{
    void *inbuf_demmy;
    size_t inbuf_len_demmy;
    size_t index;

    get_args(argc, argv, &index, &inbuf_demmy, &inbuf_len_demmy);
    ...
    if (index & 0x00000002) {
        ...
        uint8_t rsa_sign_key[] = "rsaSignKey";
        ...
        for (size_t i = 0; i < LOOP_COUNT; i++) {
            ...
            if(res == 0) {
                printf("rsa verify pass, i = %ld\n", i);
            } else {
                printf("rsa verify failed, i = %ld\n", i);
            }
    ...
    if (index & 0x00000008) {
        ...
        uint8_t rsa3072_sign_key[] = "rsaSignKey";
    ...
    // sm2 sign/verify
        ...
        for (size_t i = 0; i < LOOP_COUNT; i++) {
            ...
            if(res == 0) {
                printf("sm2 dsa verify pass, i = %ld\n", i);
            } else {
                printf("sm2 dsa verify failed, i = %ld\n", i);
            }
    ...
    if (index & 0x00000100) {
        ...
        uint32_t TMP_LOOP_COUNT = LOOP_COUNT * 100;
        for (size_t i = 0; i < TMP_LOOP_COUNT ; i++) {
            res = CryptoSm3Hash(sm3In.data, sm3In.len, (void **)&sm3Out.data, &sm3Out.len);
            free(sm3Out.data);
        }
    ...
    if (index & 0x00000200) {
        ...
        uint32_t TMP_LOOP_COUNT = LOOP_COUNT * 100;
        for (size_t i = 0; i < TMP_LOOP_COUNT ; i++) {
            res = CryptoShaHash(shaIn.data, shaIn.len, (void **)&shaOut.data, &shaOut.len);
            free(shaOut.data);
        }
